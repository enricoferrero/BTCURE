...
ensreg_promoters$coordinates <- with(ensreg_promoters, paste0(chr, ":", start, "-", end))
ensreg_promoters_list <- split(ensreg_promoters, ensreg_promoters$coordinates)

prom_reshaper <- function(promoter_df){
	cell_types <- c("CD4_VB", "CD8_CB", "EB_CB", "Mac0_CB", "Mac0_VB", "Mac1_CB", "Mac1_VB", "Mac2_CB", "Mac2_VB", "Mon_CB", "Mon_VB", "Neu_CB", "Neu_VB")
	raw_df <- data.frame(cell_types, as.character(promoter_df[match(cell_types, promoter_df$Cell_type),"Activity"]))
	colnames(raw_df) <- c("Cell_type", "Activity")
	activity_df <- data.frame(raw_df$Activity, row.names = raw_df$Cell_type)
	colnames(activity_df) <- "Activity"
	#activity_df <- data.frame(chr = unique(promoter_df$chr), t(activity_df))



	activity_df <- data.frame(chr = unique(promoter_df$chr), 
				  start = unique(promoter_df$start), 
				  end = unique(promoter_df$end), 
				  activity_df)
	rownames(activity_df) <- unique(promoter_df$coordinates)
	return(activity_df)
}
...
ensreg_promoters_list_reshaped <- parLapply(cl = cl, X = ensreg_promoters_list, fun = prom_reshaper)

stopCluster(cl)
gc()

lapply(ensreg_promoters_list_reshaped, dim)

ensreg_promoters_gr <- makeGRangesFromDataFrame(ensreg_promoters_list_reshaped, keep.extra.columns = T)
...
ensreg_promoters_gr <- makeGRangesFromDataFrame(ensreg_promoters_df, keep.extra.columns =T)
...
ensreg_promoters_bait$feature_status <- "ACTIVE"
ensreg_promoters_bait$celltype <- pcHiC_Bait_anno_df$celltype
...
#ensreg_promoters_df <- rbind(ensreg_promoters_df, ensreg_promoters_bait)
...
write.csv(as.data.frame(ensreg_actprom_anno), file.path(promoters_parsed_dir, "GRCh38_actprom_blood.csv"))
```
...
write.csv(HvL_dm_actprom, file.path(dmpromoters_dir, "HvL_dmpromoters_active.csv"))
write.csv(HvN_dm_actprom, file.path(dmpromoters_dir, "HvN_dmpromoters_active.csv"))
write.csv(LvN_dm_actprom, file.path(dmpromoters_dir, "LvN_dmpromoters_active.csv"))
```
