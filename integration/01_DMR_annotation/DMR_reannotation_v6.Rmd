...
HvL_dmrs_noSex_gr <- keepStandardChromosomes(HvL_dmrs_gr[!seqnames(HvL_dmrs_gr) %in% c("X", "Y"),], pruning.mode = "coarse")
HvN_dmrs_noSex_gr <- keepStandardChromosomes(HvN_dmrs_gr[!seqnames(HvN_dmrs_gr) %in% c("X", "Y"),], pruning.mode = "coarse")
LvN_dmrs_noSex_gr <- keepStandardChromosomes(LvN_dmrs_gr[!seqnames(LvN_dmrs_gr) %in% c("X", "Y"),], pruning.mode = "coarse")

dmrs_noSex_gr <- c(HvL_dmrs_noSex_gr, HvN_dmrs_noSex_gr, LvN_dmrs_noSex_gr)
```
...
dmrs_noSex_peaks <- annotatePeak(dmrs_noSex_gr, tssRegion = c(-2500, 250), TxDb = txdb_GRCh38, annoDb = "org.Hs.eg.db")
write.csv(dmrs_noSex_peaks@annoStat, file.path(out_dir, "DMR_feature_frequency.csv"))

pdf(file.path(out_dir, "DMR_upset.pdf"), width = 4.5, height = 4.5)
upsetplot(dmrs_noSex_peaks)
dev.off()
```
...
#Find overlapping enhancer regions and merge them
pcHiC_GRCh38_OE_split <- reduce(split(pcHiC_GRCh38_OE, elementMetadata(pcHiC_GRCh38_OE)$baitENS))
